        ...
        for i in range(start_idx, len(questions), DEFAULT_BATCH_SIZE):
            ...
            for j, (question, answer_slot) in enumerate(zip(batch, batch_results)):
                ...
                if question.get('Answer'):
                    for ans in question['Answer']:
                        if isinstance(ans, dict) and 'AnswerArgument' in ans:
                            ...
                            answers.append(ans)
                    if question['Answer'] and isinstance(question['Answer'][0], dict):
                        answer_type = question['Answer'][0].get('AnswerType', 'unknown')
                ...
                result = {
                    ...
                    "time_level": "day",  # 기본값 설정
                    "qtype": question['Temporal question type'][0] if question.get('Temporal question type') else "unknown",
                    "qlabel": "Multiple"  # 기본값 설정
                }
...
async def main():
    # train.json 파일 읽기
    input_file = "datasets/TimeQuestions/questions/test.json"
    output_file = "datasets/TimeQuestions/results/answer_slot_test_vote_1call_6samples_0.7.json"
